...
from ..assets.models import SACActor, SoftQNetwork, Transformer
...
from ..data.replay_buffer import EpisodicReplayBuffer, EpisodicReplayBufferSamples
from .adaptran import get_observation_dim


class AdaptranV3(GenericAgent):
    def __init__(
        self,
        ...
        expanse_dim: int = 256,
        seed: int = 1,
        critic_gradient_prop: bool = False,
        preloaded_replay_buffer_path: Union[PurePath, None] = None,
    ):
        super().__init__(envs)
        self.device = device
        ...
        self.seed = seed
        self.critic_gradient_prop = critic_gradient_prop
        ...
        self.flat_latent_dim = (
            self.repr_model.src_seq_length * self.repr_model.embed_dim
        )
        self.state_dim = self.flat_latent_dim  # + self.context_dim

        self.actor = SACActor(self.envs, self.state_dim, expanse_dim).to(device)
        ...
        if preloaded_replay_buffer_path:
            self.replay_buffer.load(path=preloaded_replay_buffer_path)
    ...
    def update_agent(self, experience):
        ...
        batch: EpisodicReplayBufferSamples = self.replay_buffer.sample(
            self.batch_size, desired_length=self.src_seq_length + 1
        )

        state_transitions = self.update_repr_model(batch)
    ...
    def update_repr_model(self, batch: EpisodicReplayBufferSamples):
        ...
        context_loss = F.mse_loss(context_head_output, batch.contexts[:, t, :])
        total_loss = recon_loss + context_loss

        if not self.critic_gradient_prop:
        ...
        with torch.no_grad():
            ...
            source[:, -1, -self.action_dim :] = 0
            enc_output_target, [context_head_output_target] = self.repr_model_target(
                source, enc_only=True
            )

        state_transitions: CompactStateTransitions = CompactStateTransitions(
            states=(
                torch.flatten(enc_output, start_dim=1, end_dim=2)
                if self.critic_gradient_prop
                else torch.flatten(enc_output, start_dim=1, end_dim=2).detach()
            ).to(self.device),
            # states=torch.cat(
            #     [
            ...
            #         # context_head_output
            #         # if self.critic_gradient_prop
            #         # else context_head_output.detach(),
            #         # batch.contexts[:, -1, :],
            #     ],
            #     dim=-1,
            # ).to(self.device),
            next_states=(
                torch.flatten(enc_output_target, start_dim=1, end_dim=2).detach()
            ).to(self.device),
            # next_states=torch.cat(
            #     [
            #         # batch.next_observations[:, -1, :],
            #         torch.flatten(enc_output_target, start_dim=1, end_dim=2).detach(),
            #         # context_head_output_target.detach(),
            #         # batch.contexts[:, -1, :],
            #     ],
            #     dim=-1,
            # ).to(self.device),
    ...
    def update_critic(self, batch: CompactStateTransitions):
        """
        ...
        with torch.no_grad():
            ...
            qf1_next_target = self.qf1_target(batch.next_states, next_state_actions)
            qf2_next_target = self.qf2_target(batch.next_states, next_state_actions)
        ...
        qf1_a_values = self.qf1(batch.states, batch.actions).view(-1)
        qf2_a_values = self.qf2(batch.states, batch.actions).view(-1)
    ...
    def update_actor(self, batch: CompactStateTransitions):
        """
        ...
        for _ in range(self.policy_frequency):
            ...
            if self.autotune:
                ...
                with torch.no_grad():
                    _, log_pi, _ = self.sample_action(batch.states)
    ...
    def train(
        ...
        for _ in range(total_timesteps):
            ...
            with torch.no_grad():
                ...
                state = self.get_current_state(curr_obs)
    ...
    def get_current_state(self, curr_obs: np.ndarray):
        ...
        samples: EpisodicReplayBufferSamples = self.replay_buffer.get_last_episode()
        ...
        for env_no in range(self.n_envs):
            ...
            start = ep_len - self.repr_model.src_seq_length + 1
        ...
        with torch.no_grad():
            enc_output, [context_head_output] = self.repr_model_target(
                source, enc_only=True
            )
